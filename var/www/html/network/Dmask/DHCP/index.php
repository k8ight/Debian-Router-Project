...
$filex=fopen("/etc/dhcp/dhcpd.conf","w");
fwrite($filex,$dfph);
fclose($filex);
...
<table><tr></tr>
<tr><td style="width:200px;color:red;">Manual Dhcp Assign<br>
IF LAN and OPT port ip changed<br>
Change ip range here according to those new ip. 
</td><td><textarea name="dfph" style="font-weight:900;height:400px;width:100%;resize:none;"><?php
$dfp=fopen("/etc/dhcp/dhcpd.conf","r");
while(! feof($dfp))
  {
  echo fgets($dfp);
  }
